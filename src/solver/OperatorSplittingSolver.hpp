...
namespace polyfem
{
    ...
    public:
        // initialization
        void initialize_grid(const polyfem::Mesh& mesh, const std::vector<polyfem::ElementBases>& gbases, const std::vector<polyfem::ElementBases>& bases, const double& density_dx);
        void initialize_mesh(const polyfem::Mesh& mesh, const int shape, const int n_el, const std::vector<LocalBoundary>& local_boundary);
        void initialize_hashtable(const polyfem::Mesh& mesh);
        void initialize_solver(const polyfem::Mesh& mesh, const int shape_, const int n_el_, const std::vector<LocalBoundary>& local_boundary, const std::vector<int>& bnd_nodes);

        OperatorSplittingSolver() {};
        OperatorSplittingSolver(const polyfem::Mesh& mesh, const int shape, const int n_el, const std::vector<LocalBoundary>& local_boundary, const std::vector<int>& bnd_nodes)
        { initialize_solver(mesh, shape, n_el, local_boundary, bnd_nodes); };
        OperatorSplittingSolver(const polyfem::Mesh& mesh, const int shape, const int n_el, const std::vector<LocalBoundary>& local_boundary, const std::vector<int>& boundary_nodes_, const std::vector<int>& pressure_boundary_nodes, const std::vector<int>& bnd_nodes, const StiffnessMatrix& mass, const StiffnessMatrix& stiffness_viscosity, const StiffnessMatrix& stiffness_velocity, const StiffnessMatrix& mass_velocity, const double& dt, const double& viscosity_, const std::string &solver_type, const std::string &precond, const json& params, const std::string &save_path);
        ...
        void advection(const polyfem::Mesh& mesh, const std::vector<polyfem::ElementBases>& gbases, const std::vector<polyfem::ElementBases>& bases, Eigen::MatrixXd& sol, const double dt, const Eigen::MatrixXd& local_pts, const int order = 1, const int RK = 1);
        ...
        void solve_diffusion_1st(const StiffnessMatrix& mass, const std::vector<int>& bnd_nodes, const Eigen::MatrixXd& bc, Eigen::MatrixXd& sol);
        ...
        void projection(int n_bases, const std::vector<polyfem::ElementBases>& gbases, const std::vector<polyfem::ElementBases>& bases, const std::vector<polyfem::ElementBases>& pressure_bases, const Eigen::MatrixXd& local_pts, Eigen::MatrixXd& pressure, Eigen::MatrixXd& sol);
