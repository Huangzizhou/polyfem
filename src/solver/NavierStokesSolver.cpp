...
namespace polyfem
{
	...
	void NavierStokesSolver::minimize(const State &state, const Eigen::MatrixXd &rhs, Eigen::VectorXd &x)
	{
		...
		double nlres_norm = 0;
		b = rhs;
		it += minimize_aux(state.formulation() + "Picard", skipping, state, velocity_stiffness, mixed_stiffness, pressure_stiffness, integrals, b, 1e-3, solver, nlres_norm, x);
		it += minimize_aux(state.formulation(), skipping, state, velocity_stiffness, mixed_stiffness, pressure_stiffness, integrals, b, gradNorm, solver, nlres_norm, x);
